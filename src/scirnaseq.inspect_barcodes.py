...
def get_cli_arguments():
    ...
    parser.add_argument(
        ...
        type=int)
    default = (
        "metadata/737K-cratac-v1.reverse_complement.csv")
    parser.add_argument(
        "--10x-whitelist",
        dest="bc_10x",
...
def main():
    ...
    if "reverse_complement" not in bc.columns:
        ...
        bc.loc[:, "reverse_complement"] = [str(Seq(x).reverse_complement()) for x in bc['original']]
    ...
    for mismatches in range(args.min_mismatches, args.max_mismatches):
        ...
        if args.only_whitelist:
            i = df['round2'].isin(bc['reverse_complement'].tolist())
            ic = i.sum()
            msg = " - reads with correct 10X barcodes: '{}'".format(ic)
            print("## " + time.asctime() + msg)
            msg = " - fraction of correct 10X barcodes: '{}'".format(ic / float(df.shape[0]))
            print("## " + time.asctime() + msg)
        ...
        # # per cell
        reads_per_cell = df.groupby(args.cell_barcodes)['gene'].count()  # gene here is just a dummy
        ...
        cell_rates = pd.DataFrame([reads_per_cell, unmapped, mapping_rate], index=['all_reads', 'unmapped', 'mapping_rate']).T
        ...
        axis[0, 1].plot(r.tail(args.estimated_cells), reads_per_cell.tail(args.estimated_cells))
        axis[1, 0].plot(r, reads_per_cell)
        axis[1, 0].set_xscale('log')
        axis[1, 0].set_yscale('log')
        axis[1, 1].plot(r.tail(args.estimated_cells), reads_per_cell.tail(args.estimated_cells))
        ...
        sns.despine(fig)
        fig.savefig(
            os.path.join(args.output_dir, output_prefix + "reads_per_cell.lineplot.svg"),
            bbox_inches="tight")
        ...
        duplicates_per_cell = unique_molecules.groupby(args.cell_barcodes).sum().to_frame(name="unique")
        duplicates_per_cell.loc[:, "all"] = duplicates_per_molecule.groupby(args.cell_barcodes).sum()
        ...
        axis[0, 1].plot(r.tail(args.estimated_cells), umis_per_cell.tail(args.estimated_cells))
        axis[1, 0].plot(r, umis_per_cell)
        axis[1, 0].set_xscale('log')
        axis[1, 0].set_yscale('log')
        axis[1, 1].plot(r.tail(args.estimated_cells), umis_per_cell.tail(args.estimated_cells))
        axis[1, 1].set_xscale('log')
        axis[1, 1].set_yscale('log')
        for ax in axis.flatten():
            ax.axvline(args.estimated_cells, linestyle="--", color="black", alpha=0.5)
        ...
        sp_ratio = df3.pivot_table(index=args.cell_barcodes, columns='human', values='umi', fill_value=0, aggfunc=sum)
        sp_ratio.columns = ['mouse', 'human']
        sp_ratio.loc[:, "total_umis"] = sp_ratio.sum(axis=1)
        # # normalize to mean per species
        sp_ratio = sp_ratio.assign(mouse_norm=sp_ratio['mouse'] / sp_ratio['mouse'].mean())
        sp_ratio = sp_ratio.assign(human_norm=sp_ratio['human'] / sp_ratio['human'].mean())
        ...
        sp_ratio.loc[:, 'ratio'] = sp_ratio.loc[:, 'ratio'].replace(np.inf, m).replace(-np.inf, 0)
        sp_ratio.loc[:, 'log_ratio'] = np.log2(sp_ratio.loc[:, 'ratio']).replace(-np.inf, -np.log2(m))
        ...
        msg = " - Doublet rate: {}".format((sp_ratio['species'] == 'doublet').sum() / float(sp_ratio.shape[0]))
        print("# " + time.asctime() + msg)
        ...
        fig.savefig(
            os.path.join(
                args.output_dir, output_prefix + ".species_mix.summary.dependent_purity.svg"), bbox_inches="tight", dpi=300)
        ...
        axis[0].scatter(
            sp_ratio.loc[:, "mouse"],
            sp_ratio.loc[:, "human"],
            c=sp_ratio.loc[:, 'log_ratio'], cmap="coolwarm", s=2, alpha=0.8, rasterized=True)
        axis[0].set_xlim((0, m))
        axis[0].set_ylim((0, m))
        ...
        for j, var in enumerate(["umi", "gene"]):
            for i, l in [(0, ""), (1, " (log)")]:
                ...
                axis[i, j].text(0.5e5, args.estimated_cells, s="y = {:.6f}x + {:.2f}".format(m, b), ha="center")
        ...
        r2_only = df3.groupby([args.cell_barcodes[1]] + ['gene', 'human'])['umi'].sum().reset_index()

        r2_sp_ratio = r2_only.pivot_table(index=args.cell_barcodes[1], columns='human', values='umi', fill_value=0, aggfunc=sum)
        r2_sp_ratio.columns = ['mouse', 'human']
        r2_sp_ratio.loc[:, "total_umis"] = r2_sp_ratio.sum(axis=1)
        # # normalize to mean per species
        r2_sp_ratio = r2_sp_ratio.assign(mouse_norm=r2_sp_ratio['mouse'] / r2_sp_ratio['mouse'].mean())
        r2_sp_ratio = r2_sp_ratio.assign(human_norm=r2_sp_ratio['human'] / r2_sp_ratio['human'].mean())

        r2_sp_ratio.loc[:, 'ratio'] = r2_sp_ratio.loc[:, 'human'] / r2_sp_ratio.loc[:, 'mouse']
        r2_sp_ratio.loc[:, 'ratio'] = r2_sp_ratio.loc[:, 'ratio']
        m = r2_sp_ratio.loc[:, 'ratio'].replace(np.inf, 0).replace(-np.inf, 0).abs().max()
        r2_sp_ratio.loc[:, 'ratio'] = r2_sp_ratio.loc[:, 'ratio'].replace(np.inf, m).replace(-np.inf, 0)
        r2_sp_ratio.loc[:, 'log_ratio'] = np.log2(r2_sp_ratio.loc[:, 'ratio']).replace(-np.inf, -np.log2(m))
        ...
        msg = " - Doublet rate: {}".format((r2_sp_ratio.species == 'doublet').sum() / float(r2_sp_ratio.shape[0]))
        print("# " + time.asctime() + msg)
        ...
        fig.savefig(
            os.path.join(
                args.output_dir, output_prefix + ".only_10X_barcodes.species_mix.summary.barplot.svg"), bbox_inches="tight", dpi=300)
        ...
        for ax in axis:
            ...
            ax.axvline(0, linestyle="--", color="black", alpha=0.1, zorder=100)
        m = max(
            sp_ratio.loc[:, "mouse"].max(), r2_sp_ratio.loc[:, "mouse"].max(),
            sp_ratio.loc[:, "human"].max(), r2_sp_ratio.loc[:, "human"].max())
        axis[0].plot((0, m), (0, m), linestyle="--", alpha=0.5, color="black")
        axis[0].scatter(sp_ratio.loc[:, "mouse"], sp_ratio.loc[:, "human"], s=3, alpha=0.3, rasterized=True, color=colors[0], label="round1 + 10X")
        axis[0].scatter(r2_sp_ratio.loc[:, "mouse"], r2_sp_ratio.loc[:, "human"], s=3, alpha=0.3, rasterized=True, color=colors[1], label="10X only")
        axis[0].legend()
        ...
        axis[1].plot((0, m), (0, m), linestyle="--", alpha=0.5, color="black")
        axis[1].scatter(a, b, s=3, alpha=0.3, rasterized=True, color=colors[0], label="round1 + 10X")
        axis[1].scatter(r2a, r2b, s=3, alpha=0.3, rasterized=True, color=colors[1], label="10X only")
        axis[1].legend()
        ...
        fig.savefig(
            os.path.join(
                args.output_dir, output_prefix + ".only_10X_barcodes.species_mix.plot.svg"), bbox_inches="tight", dpi=300)
        ...
        fig.savefig(
            os.path.join(
                args.output_dir, output_prefix + ".only_10X_barcodes.species_mix.summary.dependent_purity.svg"), bbox_inches="tight", dpi=300)
        ...
        for i, fold in enumerate([0.1, 0.25, 0.5, 1, 2]):
            ...
            r1_demux = r1_2_tmp.loc[r1_2_tmp.index.get_level_values("round2").isin(r2_doublets.tolist())].index

            demux_doublet_fraction = r1_2_tmp.loc[r1_demux, 'species'].value_counts()
            demux_doublet_fraction = demux_doublet_fraction['doublet'] / float(demux_doublet_fraction.sum())
            ...
            axis[i, 1].text(6, 1, s="Doublet fraction = {:2f}".format(demux_doublet_fraction))

            axis[i, 0].set_title("Doublets with 10X")
            axis[i, 0].scatter(
                r2a.loc[r2_doublets],
                r2b.loc[r2_doublets],
                s=5, alpha=0.5, rasterized=True, color=colors[1], label="10X only")
            ...
            axis[i, 1].scatter(
                a.loc[r1_demux],
                b.loc[r1_demux],
                s=5, alpha=0.5, rasterized=True, color=colors[0], label="round1 + 10X")
        sns.despine(fig)
        fig.savefig(
            os.path.join(
                args.output_dir, output_prefix + ".species_mix.plot.doublet_difference.svg"), bbox_inches="tight", dpi=300)
        ...
        cells_per_droplet = df3.groupby(args.cell_barcodes[1])[args.cell_barcodes[0]].nunique()
        ...
        lamb, cov_matrix = curve_fit(poisson, cells_per_droplet_counts.index, cells_per_droplet_counts)
        print(lamb)
        lamb, cov_matrix = curve_fit(log_likelihood, cells_per_droplet_counts.index, cells_per_droplet_counts)
        print(lamb)

        y_hat = scipy.stats.poisson.pmf(np.arange(1, 101), lamb)
        y_hat *= cells_per_droplet.shape[0]
        y_hat[y_hat < 1] = 1
        x = np.linspace(
            cells_per_droplet_counts.index.min(),
        ...
        cells_per_droplet = df3.loc[df3['name'].isin(real_cells)].groupby(args.cell_barcodes[1])[args.cell_barcodes[0]].nunique()
        ...
        y_hat *= cells_per_droplet.shape[0]
        y_hat[y_hat < 1] = 1
        x = np.linspace(1, cells_per_droplet_counts.index.max(), 100)
        for ax in axis[1, :]:
            ax.plot(x, y_hat)
        axis[1, 1].set_yscale("log")
        ...
        fig.savefig(
            os.path.join(
                args.output_dir, output_prefix + ".poissonian_properties.svg"), bbox_inches="tight", dpi=300)
