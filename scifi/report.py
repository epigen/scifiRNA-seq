...
def report_command(
    ...
    cmd += report_cmd(
        input_directory=sample_out_dir, output_directory=sample_out_dir,
        r1_attributes=r1_attributes, species_mixture=species_mixture,
        exon=False, correct_r2_barcodes=False)
    cmd += report_cmd(
        input_directory=sample_out_dir, output_directory=sample_out_dir,
        r1_attributes=r1_attributes, species_mixture=species_mixture,
        exon=True, correct_r2_barcodes=False)
    cmd += report_cmd(
        input_directory=sample_out_dir, output_directory=sample_out_dir,
        r1_attributes=r1_attributes, species_mixture=species_mixture,
        exon=False, correct_r2_barcodes=True)
    cmd += report_cmd(
        input_directory=sample_out_dir, output_directory=sample_out_dir,
        r1_attributes=r1_attributes, species_mixture=species_mixture,
        exon=True, correct_r2_barcodes=True)
    cmd += job_end()
...
def report_cmd(
        ...
        exon: bool = False,
        correct_r2_barcodes: bool = False) -> str:
    """
    """
